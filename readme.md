...
- シンプルなサイト構成に見合う様に控え目なアニメーションですが、アイコンやタイポグラフィを活用し遊び心を入れました。
- ローディングアニメーションが複数回流れる事でユーザーのストレスにならない様、セッションストレージを使用してリロード対策を行っています。（再生する場合はDevtoolsのApplication→Session Storage→accessを選択、右Clickで削除して下さい。）
